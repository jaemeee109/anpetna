...
async function me(): Promise<CartDTO> {
  // ✅ 백엔드 실제 prefix는 /anpetna/cart  (컨트롤러 @RequestMapping 확인) :contentReference[oaicite:5]{index=5}
  const resp = await http.get('/anpetna/cart');
...
async function add(payload: AddCartReq) {
  return http.post('/anpetna/cart', payload); // 경로 수정
}
...
async function update(payload: UpdateCartReq) {
  ...
  return http.put(`/anpetna/cart/${id}`, { quantity: payload.quantity }); // 경로 수정
}
...
async function remove(id: number | { itemId?: number; cartItemId?: number }) {
  ...
  return http.delete(`/anpetna/cart/${rid}`); // 경로 수정
}
