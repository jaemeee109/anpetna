...
export default function BoardEditPage({
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    if (orders.length > 0) {
      fd.append("orders", new Blob([JSON.stringify(orders)], { type: "application/json" }));
    }

    // useUpdateBoard가 FormData를 그대로 전송
    updateMut.mutate(
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {origImages.map((img) => {
              const uuid = img.uuid!;
              const raw =
                img.url ?? img.path ?? (img as any).imageUrl ?? (img as any).src ?? "";
              const src = proxiedFileUrl(raw); // ★ 여기만 변경
              return (
