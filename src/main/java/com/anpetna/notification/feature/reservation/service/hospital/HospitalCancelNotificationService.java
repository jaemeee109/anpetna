...
public class HospitalCancelNotificationService {

    private final NotificationService notificationService;

    public void notifyHospitalCancel(MemberEntity member, String memberId, VenueEntity venue, LocalDateTime at) {

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM월 dd일 HH시 mm분");
        String formattedAt = at.format(formatter);

        String title = member.getMemberName() + " 회원님, " + formattedAt + " 안펫나 동물병원 " + venue.getVenueName() + " 예약이 취소되었습니다.";

        notificationService.createAndPush(
                CreateNotificationCmd.builder()
                        .receiverMemberId(memberId)
                        .notificationType(NotificationType.RESERVATION_HOSPITAL_CANCELED)
