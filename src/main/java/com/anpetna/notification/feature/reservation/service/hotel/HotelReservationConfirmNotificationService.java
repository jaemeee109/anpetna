...
public class HotelReservationConfirmNotificationService {

    private final NotificationService notificationService;

    public void notifyHotelReservationConfirm(MemberEntity member, String memberId, VenueEntity venue, LocalDate in, LocalDate out) {

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM월 dd일");
        String formattedIn = in.format(formatter);
        String formattedOut = out.format(formatter);

        String title = member.getMemberName() + " 회원님, " + formattedIn + " ~ " + formattedOut + " 안펫나 호텔 " + venue.getVenueName() + " 예약이 확정되었습니다.";

        notificationService.createAndPush(
                CreateNotificationCmd.builder()
                        .receiverMemberId(memberId)
                        .notificationType(NotificationType.RESERVATION_HOTEL_CONFIRM)
